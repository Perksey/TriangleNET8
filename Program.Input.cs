...
namespace InputTest
{
    public class Program
    {
        ...
        static Key[] _allKeys = Enum.GetValues(typeof(Key))
            .Cast<Key>()
        ...
        public static Action OnLoad(IView window) =>
            () =>
            {
                ...
                Console.WriteLine(
                    "Now, go press buttons in the window and you'll see the feedback here."
                );
        ...
        public static Action<double> OnUpdate(IInputContext input) =>
            (dt) =>
            {
                ...
                {
                    if (
                        _scancodesDown.Contains(s)
                        != input.Keyboards.Any(x => x.IsScancodePressed(s))
                    )
                    {
                        Console.WriteLine(
                            $"[ERROR] IsScancodePressed() and event mismatch! Scancode {s}"
                        );
                    }
                }
            };
        ...
        {
            Console.WriteLine(
                $"G{gamepad.Index}> {trigger.Index} trigger moved: {trigger.Position}"
            );
        }
        ...
        {
            Console.WriteLine(
                $"G{gamepad.Index}> {thumbstick.Index} thumbstick moved: ({thumbstick.X}, {thumbstick.Y})"
            );
        }
        ...
        public static unsafe void DoConnect(IInputDevice device, bool isConnected)
        {
            ...
            {
                Console.WriteLine(
                    $"Discovered controller {gamepad.Index} (Connected: {isConnected})"
                );
                if (isConnected)
                {
                ...
                Console.WriteLine(
                    string.Join(
                        s,
                        gamepad.Buttons.Select(x =>
                            x.Index + "/" + x.Name + (x.Pressed ? "(1)" : "(0)")
                        )
                    )
                );
            ...
            {
                Console.WriteLine(
                    $"Discovered joystick {joystick.Index} (Connected: {isConnected})"
                );
                if (isConnected)
                {
                ...
                Console.WriteLine(
                    string.Join(
                        s,
                        joystick.Buttons.Select(x => x.Name + (x.Pressed ? "(1)" : "(0)"))
                    )
                );
            }
            ...
            {
                Console.WriteLine(
                    $"Discovered keyboard {keyboard.Index} (Connected: {isConnected})"
                );
                if (isConnected)
                {
        ...
        private static void KeyboardOnKeyUp(IKeyboard keyboard, Key key, int scancode)
        {
            ...
            if (key != Key.Unknown)
            {
                ...
                if (keyIndex < 0)
                    Console.WriteLine(
                        $"[ERROR] Double key up detected? K{keyboard.Index}> {key} up."
                    );
                else
            ...
            if (scancodeIndex < 0)
                Console.WriteLine(
                    $"[ERROR] Double scancode up detected? K{keyboard.Index}> {scancode} up."
                );
            else
        ...
        private static void KeyboardOnKeyDown(IKeyboard keyboard, Key key, int scancode)
        {
            ...
            if (key != Key.Unknown)
            {
                if (_keysDown.Contains(key))
                    Console.WriteLine(
                        $"[ERROR] Double key down detected? K{keyboard.Index}> {key} down."
                    );
                else
                    _keysDown.Add(key);
            }
            ...
            if (_scancodesDown.Contains(scancode))
                Console.WriteLine(
                    $"[ERROR] Double scancode down detected? K{keyboard.Index}> {scancode} down."
                );
            else
